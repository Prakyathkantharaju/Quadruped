...
path = path_ + '/' + rel_path
rel_path_2 = 'models/distance_simple_3.xml'
...
gym.envs.register(
     id='car-robot-distance-range-left',
     entry_point='car_env_4:CarEnv',
     max_episode_steps=10000,
...
def make_env_left(seed=0):
    ...
    def _init():
        ...
        env = gym.make('car-robot-distance-range-left')
        print(f"env seed: {seed}")
        return Monitor(env, info_keywords=('reward', 'distance','episode_length'), filename=f'.run_logs/logs/{run.id}_left')
...
if __name__ == '__main__':
    env_list = [make_env(0), make_env_left(0)]
    ...
    model = PPO("MlpPolicy",'car-robot-distance-range-left', tensorboard_log=f"./.run_logs/logs/{run.id}", device="cuda", normalize_advantage=True,create_eval_env=True)
