...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def check_direction(self):
        ...
        if np.mean(np.array(self.store_vel)[:-2, 0]) > 0 and abs(np.mean(np.array(self.store_vel)[:-2, 1])) < 1e-1:
            # forward
            return True if self.com_velocity[0] == 1 else False
        if np.mean(np.array(self.store_vel)[:-2, 0]) < 0 and abs(np.mean(np.array(self.store_vel)[:-2, 1])) < 1e-1:
            # backward
            return True if self.com_velocity[0] == -1 else False
        if np.mean(np.array(self.store_vel)[:-2, 1]) > 0 and abs(np.mean(np.array(self.store_vel)[:-2, 0])) < 1e-1:
            # right
            return True if self.com_velocity[1] == 1 else False
        if np.mean(np.array(self.store_vel)[:-2, 1]) < 0 and abs(np.mean(np.array(self.store_vel)[:-2, 0])) < 1e-1:
            # left
            return True if self.com_velocity[1] == -1 else False
        return False
    ...
    def is_healthy(self):
        ...
        if self.i > 400 and not self.check_direction:
    ...
    def _load_velocities(self):
        self._com_vel = np.array([[1, 0], # forward
                            [-1, 0], # backward
                            [0, 1],
                            [0, -1]])
    ...
    def com_velocity(self):
        ...
        elif 400 <= self.i < 1000:
            return self._com_vel[1, :]
        elif 800 <= self.i < 1200:
            return self._com_vel[2, :]
        elif 1200 <= self.i < 1600:
    ...
    def step(self, action):
        ...
        self.store_vel.append(xy_velocity)
        forward_reward = np.dot(xy_velocity, self.com_velocity.T)
        ...
        info = {
            ...
            "tracking_reward": forward_reward,
