...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def is_healthy(self):
        ...
        if self.i > 20 and (abs(self.xy_vel[0] - self.com_velocity[0]) > 0.2  or  abs(self.xy_vel[1] - self.com_velocity[1]) > 0.2 ):
    ...
    def com_velocity(self):
        if self.i < 30:
            return self._com_vel[0, :] - self.xy_vel
        elif 30 <= self.i < 70:
            return self._com_vel[1, :] - self.xy_vel
        elif 70 <= self.i < 200:
            return np.array([0.1, 0.0]) - self.xy_vel
        elif 200 <= self.i < 250:
            return self._com_vel[3, :] - self.xy_vel
        elif 250 <= self.i < 300:
            return self._com_vel[4, :] - self.xy_vel
        elif 300 <= self.i < 350:
            return self._com_vel[5, :] - self.xy_vel
        elif 350 <= self.i < 400:
            return self._com_vel[0, :] - self.xy_vel
        elif 400 <= self.i < 450:
            return self._com_vel[2, :] - self.xy_vel
        elif 450 <= self.i < 550:
            return self._com_vel[4, :] - self.xy_vel
        elif 500 <= self.i < 650:
            return self._com_vel[3, :] - self.xy_vel
